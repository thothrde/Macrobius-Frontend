...
export const MacrobiusAPI = {
  system: {
    healthCheck: (): Promise<ApiResponse<{status: string, performance: PerformanceMetrics}>> => apiClient.healthCheck()
  },
  passages: {
    ...
    getCorpusStatistics: (): Promise<ApiResponse<any>> =>
      apiClient.request<ApiResponse<any>>('/api/passages/statistics'),
    searchPassages: (query: string, filters: SearchFilters): Promise<ApiResponse<PassagesResponse>> =>
      apiClient.request<ApiResponse<PassagesResponse>>('/api/passages/search', {
        method: 'POST',
        body: { query, ...filters }
      }),
    getPassage: (passageId: number): Promise<ApiResponse<{passage: MacrobiusPassage}>> =>
      apiClient.request<ApiResponse<{passage: MacrobiusPassage}>>(`/api/passages/${passageId}`)
  },
  text: {
    search: (query: string, language?: string, category?: string, page?: number) =>
      apiClient.searchTexts(query, language, category, page)
  },
  quiz: {
    getCategories: () => apiClient.getQuizCategories(),
    getQuizzes: (categoryId?: number, language?: string) => apiClient.getQuizzes(categoryId, language),
    generateCulturalQuiz: (insightIds: string[], difficulty: string, count: number): Promise<ApiResponse<any>> =>
      apiClient.request<ApiResponse<any>>('/api/quiz/cultural/generate', {
        method: 'POST',
        body: { insightIds, difficulty, count }
      }),
    submitQuizResults: (quizId: string, answers: Record<string, string>): Promise<ApiResponse<any>> =>
      apiClient.request<ApiResponse<any>>('/api/quiz/submit', {
        method: 'POST',
        body: { quizId, answers }
      })
  },
  cultural: {
    ...
    getInsights: (themeId?: string): Promise<ApiResponse<{insights: CulturalInsight[]}>> =>
      apiClient.request<ApiResponse<{insights: CulturalInsight[]}>>(`/api/cultural/insights${themeId ? `?theme=${themeId}` : ''}`)
  },
  vocabulary: {
    ...
    getHighFrequencyWords: (count: number): Promise<ApiResponse<{vocabulary: MacrobiusVocabulary[]}>> =>
      apiClient.request<ApiResponse<{vocabulary: MacrobiusVocabulary[]}>>(`/api/vocabulary/high-frequency?count=${count}`),
    searchVocabulary: (query: string, filters: any): Promise<ApiResponse<{vocabulary: MacrobiusVocabulary[]}>> =>
      apiClient.request<ApiResponse<{vocabulary: MacrobiusVocabulary[]}>>('/api/vocabulary/search', {
        method: 'POST',
        body: { query, ...filters }
      }),
    submitVocabularyProgress: (wordId: number, correct: boolean): Promise<ApiResponse<any>> =>
      apiClient.request<ApiResponse<any>>('/api/vocabulary/progress', {
        method: 'POST',
        body: { wordId, correct }
      })
  }
...
// Type definitions for passages and vocabulary
...
// Types are exported where they are defined