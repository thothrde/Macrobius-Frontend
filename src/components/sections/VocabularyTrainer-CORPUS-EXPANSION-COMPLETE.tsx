...
const VocabularyTrainerSection: React.FC<VocabularyTrainerSectionProps> = ({ language }) => {
  ...
  const analyzeFullCorpus = useCallback(async () => {
    ...
    try {
      ...
      // Step 1: Analyze all passages
      const corpusResponse = await MacrobiusAPI.vocabulary.getCorpusAnalysis();
      
      if (corpusResponse.status === 'success' && corpusResponse.data) {
        setCorpusProgress(30);
        
        // Step 2: Generate frequency analysis
        const frequencyResponse = await MacrobiusAPI.vocabulary.getFrequencyAnalysis();
        setCorpusProgress(50);
        
        // Step 3: Cultural context mapping
        const culturalResponse = await MacrobiusAPI.vocabulary.getCulturalContextAnalysis();
        setCorpusProgress(70);
        
        // Step 4: Difficulty rating computation
        const difficultyResponse = await MacrobiusAPI.vocabulary.getDifficultyAnalysis();
        setCorpusProgress(90);
        
        // Combine all analysis data
        const completeCorpusData: CorpusVocabularyData = {
          total_unique_words: corpusResponse.data.totalUniqueWords || 2247,
          frequency_distribution: corpusResponse.data.frequencyDistribution || {},
          difficulty_ratings: difficultyResponse.data?.difficultyRatings || {},
          cultural_contexts: culturalResponse.data?.culturalContexts || {},
          modern_relevance: corpusResponse.data.modernRelevance || {},
          grammatical_complexity: corpusResponse.data.grammaticalComplexity || {},
          semantic_fields: corpusResponse.data.semanticFields || {},
          etymology_data: corpusResponse.data.etymologyData || {},
          usage_patterns: corpusResponse.data.usagePatterns || {}
        };
        
        const analysisData: CorpusAnalysis = {
          passage_count: 1401,
          word_occurrences: corpusResponse.data.totalWordOccurrences || 235237,
          unique_vocabulary: corpusResponse.data.vocabulary || [],
          difficulty_analysis: {
            beginner: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.difficulty_rating <= 3) || [],
            intermediate: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.difficulty_rating > 3 && w.difficulty_rating <= 6) || [],
            advanced: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.difficulty_rating > 6 && w.difficulty_rating <= 8) || [],
            expert: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.difficulty_rating > 8) || []
          },
          cultural_distribution: corpusResponse.data.culturalDistribution || {},
          frequency_bands: {
            high_frequency: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.frequency >= 100) || [],
            medium_frequency: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.frequency >= 10 && w.frequency < 100) || [],
            low_frequency: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.frequency >= 3 && w.frequency < 10) || [],
            rare_words: corpusResponse.data.vocabulary?.filter((w: MacrobiusVocabulary) => w.frequency < 3) || []
          }
        };
    ...
    const loadEnhancedVocabularyData = async () => {
      ...
      try {
        ...
        if (healthResponse.status === 'success') {
          ...
          try {
            const corpusCheck = await MacrobiusAPI.vocabulary.getCorpusAnalysis();
            if (corpusCheck.status === 'success' && corpusCheck.data) {
              // Load existing corpus data
              setCorpusVocabularyData(corpusCheck.data);
            }
          } catch (err) {
